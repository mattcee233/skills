...
2. **Name the harness you are running inside**, meaning the application and not your model, as one of `claude-code`, `antigravity`, `pithagoras` or `other`, with a one-line reason naming your evidence. Say `other` when unsure. Run `node <skill>/bridge/session.js detect --harness <id> --reason "<evidence>"`.
