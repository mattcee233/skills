...
test('teach is still user-invoked only', { skip: !inRepo }, () => {
...
test('the docs page has the four fixed sections in order, and covers interactive mode', { skip: !inRepo }, () => {
...
test('teach is still listed in the READMEs and the plugin manifest', { skip: !inRepo }, () => {
...
test('a changeset describes the interactive change', { skip: !inRepo }, () => {
  const dir = path.join(REPO, '.changeset');
  const changesets = fs.readdirSync(dir).filter((f) => f.endsWith('.md') && f !== 'README.md');
  const found = changesets.map((f) => fs.readFileSync(path.join(dir, f), 'utf8')).find((text) => /interactive lessons/i.test(text));
  assert.ok(found, 'a changeset mentions interactive lessons');
  assert.match(found, /^"mattpocock-skills": minor$/m);
