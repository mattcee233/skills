...
- Free-text quiz questions: a text box with a hidden model answer. With chat connected, the lesson button sends the learner's words to the teacher to grade (the server labels it as an answer to grade) instead of revealing the model answer; without chat it reveals it as before.
